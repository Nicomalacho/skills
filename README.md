...
- [`pr-review-watch/`](./pr-review-watch/) — poll a Slack channel for PR-review mentions and, per PR link, spin up a full local review session: a [feature-cli](https://github.com/Nicomalacho) worktree via `feature from-pr`, a light-green `[PR-REVIEW]` cmux workspace running a Claude Code session pre-prompted to `/review` the PR, and a browser tab on the PR. Tears everything down once the PR is approved by you or merged/closed. Designed for `/loop 5m` use; state lives in a local `state.json` (created at runtime, not in this repo). **⚠️ Warning:** the SKILL.md hardcodes machine/account specifics (Slack channel + user ID, GitHub login, feature-cli paths) — you **must** edit the "Hardcoded environment facts" table before running it on another machine or account, or it will watch the wrong channel and check approvals against the wrong user.
- [`machine-monitor/`](./machine-monitor/) — scan your Mac for resource pressure and runaway/orphaned dev processes (node/jest/vite/webpack/tsc/docker dev servers, duplicate port listeners, zombies, and feature-cli workspaces whose branch is already merged/deleted), classify each as keep / suspect / safe-to-kill with a reason, and — with confirm-before-every-kill — propose a numbered kill list and wait for your explicit yes/no before touching anything. Designed for `/loop` use (e.g. `/loop 5m /machine-monitor`). **Personalized** — assumes feature-cli + `~/features/`; adjust the environment-facts table before reuse.
